...
Commands:
  scaffold-native <project_dir> [name]
                                       Create a minimal native WeChat Mini Program
...
cmd_scaffold_native() {
  local dir="${1:-}"
  local app_name="${2:-Miniapp}"
  if [[ -z "$dir" ]]; then
    echo "scaffold-native requires <project_dir> [name]" >&2
    exit 2
  fi

  mkdir -p "$dir"
  local abs
  abs="$(cd "$dir" >/dev/null && pwd)"
  if [[ -e "$abs/project.config.json" || -e "$abs/app.json" || -d "$abs/pages" ]]; then
    echo "refusing to scaffold over an existing miniapp project: $abs" >&2
    exit 1
  fi

  mkdir -p "$abs/pages/index"
  node - "$abs" "$app_name" <<'NODE'
const fs = require('fs');
const path = require('path');

const [root, appName] = process.argv.slice(2);
...
writeJson('project.config.json', {
  miniprogramRoot: '.',
  appid: 'touristappid',
  projectname: appName.replace(/\s+/g, '-').toLowerCase(),
  ...
  packOptions: {
    ignore: [
      ...
      { type: 'file', value: 'pnpm-lock.yaml' },
      { type: 'file', value: 'yarn.lock' },
...
writeJson('sitemap.json', {
  desc: 'Default sitemap generated by miniapp-dev-release skill.',
  rules: [{ action: 'allow', page: '*' }],
});

writeJson('pages/index/index.json', {
  navigationBarTitleText: appName,
});
write('pages/index/index.wxml', `<view class="page">\n  <view class="title">${appName}</view>\n  <view class="subtitle">{{message}}</view>\n  <button class="primary" bindtap="onPrimaryTap">开始</button>\n</view>\n`);
write('pages/index/index.wxss', `.page {\n  min-height: 100vh;\n  padding: 48rpx 32rpx;\n  box-sizing: border-box;\n}\n\n.title {\n  font-size: 44rpx;\n  font-weight: 700;\n  line-height: 1.25;\n  margin-bottom: 18rpx;\n}\n\n.subtitle {\n  font-size: 28rpx;\n  line-height: 1.6;\n  color: #5f6b7a;\n  margin-bottom: 36rpx;\n}\n\n.primary {\n  width: 100%;\n  background: #07c160;\n  color: #fff;\n}\n`);
write('pages/index/index.js', `Page({\n  data: {\n    message: '小程序骨架已就绪，可以开始开发。',\n  },\n\n  onPrimaryTap() {\n    wx.showToast({ title: 'Ready' });\n  },\n});\n`);
NODE

  echo "native_miniapp=$abs"
  echo "appid=touristappid"
  echo "next=$(script_path) doctor \"$abs\""
}
...
main() {
  ...
  shift || true
  case "$cmd" in
    scaffold-native) cmd_scaffold_native "$@" ;;
    doctor) cmd_doctor "$@" ;;
    install) cmd_install "$@" ;;
    build) cmd_build "$@" ;;
    check) cmd_check "$@" ;;
